...
RC IndexScanPhysicalOperator::next()
{
  ...
  for(;;) {
      status = index_scanner_->next_entry(&rid, false);
      if(status != RC::SUCCESS) {
          break;
      }
      
      if((status = record_handler_->get_record(record_page_handler_, &rid, readonly_, &current_record_)) != RC::SUCCESS) {
          break;
      }

      std::vector<Record *> record_container;
      record_container.emplace_back(&current_record_);
      tuple_.set_record(record_container);
      
      if((status = filter(tuple_, pass_filter)) != RC::SUCCESS) {
          break;
      }
