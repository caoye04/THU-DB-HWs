...
RC Table::insert_record(Record &record)
{
  ...
  // [lab2] if failed ——>回滚操作：删除插入索引项目、删除插入的记录
  if (rc != RC::SUCCESS) {
    RC rc1 = delete_entry_of_indexes(record.data(), record.rid(), false );
    if (rc1 != RC::SUCCESS) {
      LOG_ERROR("Insert index entries failed. table name=%s, rc=%d:%s",
                name(), rc1, strrc(rc1));
    }
    rc1 = record_handler_->delete_record(&record.rid());
    if (rc1 != RC::SUCCESS) {
      LOG_PANIC("Insert index entries failed. table name=%s, rc=%d:%s",
                name(), rc1, strrc(rc1));
    }
  }
...
RC Table::delete_record(const Record &record)
{
  ...
  for (Index *index : indexes_) {
      ...
      if (rc != RC::SUCCESS) {
          LOG_ERROR("Failed to delete index entry. table=%s, index=%s",
