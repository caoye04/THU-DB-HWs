    ...
    SelectStmt *select_stmt, unique_ptr<LogicalNode> &logical_node)
{
  ...
  vector<unique_ptr<LogicalNode>> table_scan_nodes;
  const vector<string> &table_alias_list = select_stmt->table_alias();

  // 构建表扫描节点
  for (size_t idx = 0; idx < tables.size(); ++idx) {
    Table *current_table = tables[idx];
    vector<Field> selected_fields;

    // 筛选相关字段
    for (Field *field_ptr : all_fields) {
      if (strcmp(field_ptr->table_name(), current_table->name()) == 0) {
        selected_fields.push_back(*field_ptr);
      }
    }

    // 生成扫描节点
    table_scan_nodes.push_back(
      unique_ptr<LogicalNode>(
        new TableGetLogicalNode(
          current_table,
          table_alias_list[idx],
          selected_fields,
          true
        )
      )
  ...
  // 构建JOIN树
  if (tables.size() == 1) {
    root = move(table_scan_nodes[0]);
  } else {
    root = move(table_scan_nodes[0]);
    
    // 处理多表连接
    for (size_t i = 1; i < tables.size(); ++i) {
      unique_ptr<JoinLogicalNode> join_node(new JoinLogicalNode());
      join_node->add_child(move(root));
      join_node->add_child(move(table_scan_nodes[i]));

      // 查找合适的JOIN条件
      bool found_valid_condition = false;
      for (FilterStmt* join_filter : select_stmt->join_filter_stmts()) {
        bool involves_current = false;
        bool involves_previous = false;

        // 遍历过滤条件
        for (FilterUnit* filter_unit : join_filter->filter_units()) {
          
          // 处理左表达式
          if (filter_unit->left_expr() && 
              filter_unit->left_expr()->type() == ExprType::FIELD) {
            FieldExpr* left_field = static_cast<FieldExpr*>(filter_unit->left_expr());
            
            if (strcmp(left_field->table_name(), tables[i]->name()) == 0) {
              involves_current = true;
            } else {
              for (size_t j = 0; j < i; ++j) {
                if (strcmp(left_field->table_name(), tables[j]->name()) == 0) {
                  involves_previous = true;
                  break;
                }
              }
            }
          ...
          // 处理右表达式
          if (filter_unit->right_expr() && 
              filter_unit->right_expr()->type() == ExprType::FIELD) {
            FieldExpr* right_field = static_cast<FieldExpr*>(filter_unit->right_expr());
            
            if (strcmp(right_field->table_name(), tables[i]->name()) == 0) {
              involves_current = true;
            } else {
              for (size_t j = 0; j < i; ++j) {
                if (strcmp(right_field->table_name(), tables[j]->name()) == 0) {
                  involves_previous = true;
                  break;
                }
              }
        ...
        // 设置JOIN条件
        if (involves_current && involves_previous) {
          auto condition = _transfer_filter_stmt_to_expr(join_filter);
          if (condition) {
            join_node->set_condition(move(condition));
      ...
      if (!found_valid_condition) {
        LOG_WARN("Join condition not found for table: %s", tables[i]->name());
      }
