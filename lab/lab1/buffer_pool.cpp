...
RC FileBufferPool::flush_page_internal(Frame &frame)
{
  lock_.lock();
//  1. 获取页面Page
  Page &page_ = frame.page();
//  2. 计算该Page在文件中的偏移量
  int64_t page_offset = ((int64_t)frame.page_num()) * BP_PAGE_SIZE;
//  3. 写入数据到文件的目标位置
  if (lseek(file_desc_, page_offset, SEEK_SET) == -1) {
    return RC::IOERR_SEEK;
  }
  if (writen(file_desc_, (char*)&page_, BP_PAGE_SIZE) != 0) {
...
RC FileBufferPool::evict_all_pages()
{
  ...
  frames = frame_manager_.find_list(file_desc_);
  RC rc = RC::SUCCESS;
  // 遍历所有帧进行驱逐
  for (auto iter = frames.begin(); iter != frames.end(); iter++) {
    rc = evict_page((*iter)->page_num(), *iter);
    if (rc != RC::SUCCESS) {
      return rc;
    }
  }
...
{
  // 1. 在文件描述符到缓冲池的映射表中查找对应的缓冲池
  auto iter = fd_buffer_pools_.find(frame.file_desc());
  // 2. 如果找不到对应的缓冲池，说明是内部错误
  if (iter == fd_buffer_pools_.end()) {
    LOG_TRACE("the buffer pool was not in fd buffer pools: file_desc:%d", frame.file_desc());
    lock_.unlock();
    return RC::INTERNAL;
  }
