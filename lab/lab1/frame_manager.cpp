 ...
 * 函数分析：
 * 1. 我们找到在buffer_pool.cpp的allocate_frame函数中调用了这个函数
 * 2. 其主要功能就是当alloc分配失败（即没有空闲帧）时，调用evict_frames驱逐一个帧
 * 3. 对应到这个代码中可以发现，count就是驱逐几个帧，evict_action对应驱逐动作的回调函数
 * 
 */
int FrameManager::evict_frames(int count, std::function<RC(Frame *frame)> evict_action)
{
  ...
  // step3：开始实际驱逐
  int count_ = 0;
   for (auto iter = frames.begin(); iter != frames.end() && count_ < count; iter++) {
    RC ret = evict_action(*iter);
    
    if (ret == RC::SUCCESS) {
      frames_.remove((*iter)->frame_id());
      allocator_.free(*iter);
      count_++;
    }
  }

  return count_;  // 最终返回实际驱逐的帧数量
}
